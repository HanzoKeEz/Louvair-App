const config = {
	name: 'Louvair Store',
	handle: '@louvair',
	url: 'https://store.louvair.com',
	ogImage:
		'https://og-image.vercel.app/Store.png?theme=light&md=1&fontSize=100px&images=https%3A%2F%2Fassets.vercel.com%2Fimage%2Fupload%2Ffront%2Fassets%2Fdesign%2Fvercel-triangle-black.svg',
	description: 'Beautifully designed E-Commerce store built with Radix UI and Tailwind CSS.',
	links: {
		twitter: 'https://twitter.com/louvair',
		github: 'https://github.com/louvair',
	},
}

export default config
