const products = [
  {
    products: [
      ...
      },
      {
        ...
        description: ['Appointment scheduling', 'Patient notification', 'Create up to one office'],
        active: false
      },
      {
        name: 'Pro Plan',
  ...
  },
  {
    products: [
      ...
      },
      {
        ...
        type: 'month',
        price: '110',
        productId: 'price_1Osc5CHBei99gHwdbhfJiVAw',
        description: [
          'Appointment scheduling',
          'Patient notification',
          'Create up to one office',
    ...
    return (
      <div
        className={`p-10 border-2 hover:cursor-pointer ${
          selectedPlan.plan === product.productId ? '-translate-y-2' : 'hover:-translate-y-2'
        } transition-all w-full max-w-[21rem] min-h-[22rem] bg-black`}
        onClick={() => selectedPlan.setPlan(product.productId)}
      >
        <div className='font-bold text-3xl mb-2 capitalize'>{product.name} Plan</div>
