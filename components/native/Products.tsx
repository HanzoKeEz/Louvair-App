		...
		<Link className='' href={`/products/${product.id}`}>
			<Card className='h-full'>
				...
				<CardContent className='grid gap-1 p-4'>
					...
					<p className='text-xs text-neutral-500 text-justify'>{product.description}</p>
				</CardContent>
				<CardFooter>{product?.isAvailable ? <Price /> : <Badge variant='secondary'>Out of stock</Badge>}</CardFooter>
			</Card>
		</Link>
	)
}
